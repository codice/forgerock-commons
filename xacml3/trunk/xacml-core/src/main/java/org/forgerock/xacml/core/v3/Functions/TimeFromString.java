 ...
 * urn:oasis:names:tc:xacml:3.0:function:time-from-string
 */
public class TimeFromString extends XACMLFunction {

    public TimeFromString()  {
    }
    ...
    public FunctionArgument evaluate( XACMLEvalContext pip) throws XACML3EntitlementException {
        ...
        if (value.length() < 8) {  // Must be at least HH:mm:SS
            throw new XACML3EntitlementException("Syntax Error, No Value", URN_SYNTAX_ERROR);
        }
        try {
            Date result = XACML3PrivilegeUtils.stringToTime(value);
            return new DataValue(DataType.XACMLTIME, result, true);
        } catch(Exception e) {
