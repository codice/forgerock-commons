 ...
 * urn:oasis:names:tc:xacml:3.0:function:boolean-from-string
 */
public class BooleanFromString extends XACMLFunction {

    public BooleanFromString()  {
    }
    ...
    public FunctionArgument evaluate( XACMLEvalContext pip) throws XACML3EntitlementException {
        ...
        try {
            if ( (value.equalsIgnoreCase("yes")) || (value.equalsIgnoreCase("1")) ) {
                  value = "true";
            } else if ( (value.equalsIgnoreCase("no")) || (value.equalsIgnoreCase("0")) ) {
                  value = "false";
            }
            boolean result = Boolean.parseBoolean(value);
            return new DataValue(DataType.XACMLBOOLEAN, result, true);
        } catch(Exception e) {
