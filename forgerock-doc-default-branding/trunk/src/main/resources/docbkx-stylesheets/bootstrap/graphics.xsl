...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                exclude-result-prefixes="xlink db ng exsl exslt stext xtext lxslt simg ximg"
 ...
 <xsl:template name="process.image">
  ...
  <xsl:variable name="viewport">
   <xsl:choose>
    ...
    <xsl:when test="local-name(.) = 'inlinegraphic'
                      or ancestor::db:inlinemediaobject
  ...
  <xsl:if test="$tag = 'img' and ../../self::db:imageobjectco and not($scaled)">
   <xsl:variable name="mapname">
    <xsl:call-template name="object.id">
     <xsl:with-param name="object" select="../../db:areaspec"/>
    </xsl:call-template>
   </xsl:variable>
 ...
 <xsl:template name="process.image.attributes">
  ...
  <xsl:if test="$longdesc != ''">
   <xsl:attribute name="title">
    <xsl:value-of select="ancestor::db:imageobject/parent::*/db:textobject"/>
   </xsl:attribute>
  </xsl:if>
 ...
 <xsl:template match="db:mediaobject/db:alt">
  <xsl:apply-templates/>
 </xsl:template>
