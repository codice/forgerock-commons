...
(function($) {
    ...
    var combinations = {
        ...
        "month"  	 : /^(\d{1,2}\s){3}\*\s\*$/,            // "? ? ? * *"
        "year"   	 : /^(\d{1,2}\s){4}\*$/,                // "? ? ? ? *"
        '(n) minutes': /^\*\/\d{1,2}\s(\*\s){3}\*$/,        // "*/(n) * * * *"
        '(n) hours'  : /^0\s\*\/\d{1,2}\s(\*\s){2}\*$/,    // "0 */(n) * * *"
        '(n) days'   : /^0\s0\s\*\/\d{1,2}\s\*\s\*$/,    // "0 0 */(n) * *"
        '(n) months' : /^0\s0\s1\s1\/\d{1,2}\s\*$/         // "0 0 0 1 1/(n)"
    };
    ...
    function getCronType(cron_str) {
        ...
        var valid_cron = /^(((\d{1,2}|\*)|((\*|\d{1,2})\/\d{1,2}|\*))\s){4}((\d{1,2}|\*)|(\*\/\d{1,2}))$/;
    ...
    var methods = {
        ...
        convertCronVal: function(cronVal){
            ...
            if (cronVal.split(" ").length === 6 || cronVal.split(" ").length === 7 ) {
                cronVal = cronVal.replace('?',"*");
                expressionPieces  = cronVal.split(" ");
                expressionLength = expressionPieces.length;

                if (expressionLength === 7) {
                    expressionPieces = expressionPieces.slice(0, 6);
                }
                expressionPieces = expressionPieces.slice(1);

                if (_(parseInt(expressionPieces[4], 10)).isNumber() && !_(parseInt(expressionPieces[4], 10)).isNaN()) {
                    expressionPieces[4] = parseInt(expressionPieces[4], 10) - 1
                }
            } else {
                expressionPieces  = cronVal.split(" ");
                expressionLength = expressionPieces.length;

                expressionPieces.unshift("0");
