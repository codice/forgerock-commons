...
import org.forgerock.jaspi.container.initialisation.AuthConfigProviderLoaderFactory;
...
public class AuthConfigFactoryImpl extends AuthConfigFactory {
    ...
    private final static Logger logger = LoggerFactory.getLogger(AuthConfigFactoryImpl.class);

    private AuthConfigProviderMap authConfigProviderMap = new AuthConfigProviderMap();
    private AuthConfigProviderRegistrationIdGenerator registrationIdGenerator =
            new AuthConfigProviderRegistrationIdGenerator();

    /**
     * Required public zero arg constructor, to construct an instance of the AuthConfigFactory.
     *
     * Uses a AuthConfigProviderLoaderFactory to load the AuthConfigProvider registrations from a persistent source.
     * @throws AuthException If there is a problem loading the AuthConfigProvider registrations.
     */
    public AuthConfigFactoryImpl() throws AuthException {
        AuthConfigProviderLoader authConfigProviderLoader =
                AuthConfigProviderLoaderFactory.getAuthConfigProviderLoader();
        try {
            authConfigProviderLoader.loadAuthConfigProviders(this);
        } catch (IOException e) {
            throw new AuthException("Unable to register AuthConfigProviders from persistent source");
        }
    }
     ...
     */
    static class AuthConfigProviderRegistrationIdGenerator {
