...
public class ServerAuthContextImpl implements ServerAuthContext {
    ...
    public AuthStatus secureResponse(MessageInfo messageInfo, Subject serviceSubject) throws AuthException {
        AuthStatus authStatus = null;
        for (ServerAuthModule serverAuthModule : serverAuthModules) {
            authStatus = serverAuthModule.secureResponse(messageInfo, serviceSubject);
            if (AuthStatus.SEND_SUCCESS.equals(authStatus)) {
                // The module has successfully completed securing the response.
                continue;
            } else if (AuthStatus.SEND_FAILURE.equals(authStatus)) {
                // The module has failed to secure the response
                break;
            } else if (AuthStatus.SEND_CONTINUE.equals(authStatus)) {
                // The module has not completed securing the response.
                break;
            }
        }
        return authStatus;
